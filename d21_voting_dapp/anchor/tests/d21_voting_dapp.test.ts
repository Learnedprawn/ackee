...
describe('d21_voting_dapp', () => {
  ...
  describe('Initialize Election', () => {
    it('Should initialize election successfully', async () => {
      const election: Election = {
        ...
        candidate1: candidate1.publicKey,
        candidate1Votes: new BN(0),
        candidate2: candidate2.publicKey,
        candidate2Votes: new BN(0),
        // candidateList: [],
      }
  ...
  describe('Initialize Candidate', () => {
    ...
    it('Should initialize candidate successfully', async () => {
      ...
      const [electionPda] = getElectionPda(program, election_candidate.electionId)
      ...
      //     voteCount: new BN(0),
      // }
      await initializeCandidate(program, candidate1, electionPda, election_candidate.electionId)

      // const accountInfo = await program.provider.connection.getAccountInfo(candidatePda);
      // console.log("Account Info: ", accountInfo);
      verifyCandidate(program, election_candidate, candidate1.publicKey)
    })
  })
  ...
  describe('Vote', () => {
    it('Should allow a voter to vote for a candidate', async () => {
      ...
      //     voteCount: new BN(0),
      // };
      await initializeCandidate(program, candidate1, electionPda, election_vote.electionId)
      ...
      assert.equal(electionAccount.candidate1Votes.toString(), '1')
    })
  })
  ...
  describe('Calculate Result', () => {
    it('Should calculate correct result', async () => {
      ...
      const voter2Pda = await initializeVoter(program, voter2_struct, voter2, electionPda, result_election.electionId)
      ...
      const voter3Pda = await initializeVoter(program, voter3_struct, voter3, electionPda, result_election.electionId)
      ...
      // 6. Fetch candidate account to verify vote count
      // const electionAccount = await program.account.election.fetch(electionPda)
      // // const candidate2Account = await program.account.candidate.fetch(candidate2Pda)
      // // assert.equal(candidate1Account.voteCount.toString(), '2')
      // assert.equal(electionAccount.candidate1Votes.toString(), '2')
      // assert.equal(electionAccount.candidate2Votes.toString(), '1')

      await program.methods
        .calculateResult(result_election.electionId)
        .accounts({
          anyone: result_caller.publicKey,
          election: electionPda,
          systemProgram: anchor.web3.SystemProgram.programId,
        })
        .signers([result_caller])
        .rpc()
      ...
      const winner = electionAccount.winner.toString()
      console.log('winner: ', winner)
      assert.equal(winner, candidate1.publicKey.toString())
    })
  })
})
...
async function initializeElection(
  ...
  await program.methods
    .initializeElection(
      ...
      election.electionDesciption,
      election.electionFee,
      election.electionOrganizer,
      election.startDate,
      election.endDate,
    )
    .accounts({
...
interface Election {
  ...
  candidate1Votes: anchor.BN
  candidate2: PublicKey
  candidate2Votes: anchor.BN
}
function getElectionPda(program: Program<D21VotingDapp>, electionId: BN): [PublicKey, number] {
  return anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('election'), electionId.toArrayLike(Buffer, 'le', 8)],
    program.programId,
  )
}
async function verifyElection(program: Program<D21VotingDapp>, election: Election) {
  const [electionPda] = getElectionPda(program, election.electionId)
  const electionAccount = await program.account.election.fetch(electionPda)
...
async function initializeCandidate(
  ...
  await program.methods
    .initializeCandidate(electionId)
    .accounts({
...
async function verifyCandidate(program: Program<D21VotingDapp>, election: Election, candidate: PublicKey) {
  const [electionPda] = getElectionPda(program, election.electionId)
  // const [candidatePda] = getCandidatePda(program, electionPda, candidate.candidate)
  const electionAccount = await program.account.election.fetch(electionPda)

  assert.equal(electionAccount.candidate1.toString(), candidate.toString())
...
async function verifyVoter(program: Program<D21VotingDapp>, electionPda: PublicKey, voter: Voter) {
