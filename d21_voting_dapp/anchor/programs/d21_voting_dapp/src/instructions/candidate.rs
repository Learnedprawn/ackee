use anchor_lang::prelude::*;

use crate::{
    error::ElectionError,
    state::{Election, MAX_NAME_LEN},
};

const DISCRIMINANT: usize = 8;

pub fn _initialize_candidate(
    ctx: Context<CandidateContext>,
    // candidate_name: String,
    _election_id: u64,
) -> Result<()> {
    // let candidate_account = &mut ctx.accounts.candidate_account;
    require!(
        ctx.accounts.election.end_date >= Clock::get()?.unix_timestamp,
        ElectionError::RegistrationAfterEndDate
    );
    // require!(
    //     candidate_name.len() <= MAX_NAME_LEN,
    //     ElectionError::NameTooLong
    // );
    require!(
        ctx.accounts.election.candidate1 != ctx.accounts.candidate.key(),
        ElectionError::CannotRegisterAgain
    );

    if ctx.accounts.election.candidate1 == Pubkey::default() {
        ctx.accounts.election.candidate1 = ctx.accounts.candidate.key();
        ctx.accounts.election.candidate1_votes = 0;
    } else if ctx.accounts.election.candidate2 == Pubkey::default() {
        ctx.accounts.election.candidate2 = ctx.accounts.candidate.key();
        ctx.accounts.election.candidate2_votes = 0;
    } else {
        panic!("Both candidates filled")
    }

    // candidate_account.name = candidate_name;
    // candidate_account.election = ctx.accounts.election.key();
    // candidate_account.candidate = ctx.accounts.candidate.key();
    // candidate_account.vote_count = 0;

    Ok(())
}

#[derive(Accounts)]
#[instruction(election_id: u64)]
pub struct CandidateContext<'info> {
    #[account(mut)]
    pub candidate: Signer<'info>,
    #[account(
        mut,

        // TODO: WTFFFFFFFFFFFFFFF is this for. Took too much time to figure out. When to use this
        // and when to not should be understood later.
        // seeds = [b"election", election_id.to_le_bytes().as_ref()],
        // bump
    )]
    pub election: Account<'info, Election>,
    // #[account(
    //     init,
    //     payer = candidate,
    //     space = DISCRIMINANT + Candidate::INIT_SPACE,
    //     seeds = [b"candidate", election.key().as_ref(), candidate.key().as_ref()],
    //     bump
    // )]
    // pub candidate_account: Account<'info, Candidate>,
    pub system_program: Program<'info, System>,
}
